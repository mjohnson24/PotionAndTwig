...
Shopify.onItemAdded = function(line_item) {
  console.log('YEP!!!!');
  // if the diffuser is added, then redirect to the scent selection page
  if(line_item.id==39366126403782) {
   	//window.location.href=''; 
    console.log("ADDED DIFFUSER - REDIRECT");
    window.location.replace("/collections/diffuser-aromas");
  } else {
    console.log("NOT THE DIFFUSER");
  }
  /**
...
Shopify.onCartUpdate = function(cart) {
  Shopify.cartUpdateInfo(cart, '.mini_cart');
};
    ...
    if (cart_cell.length) {
      ...
      jQuery.each(cart, function(key, value) {
        if (key === 'items') {
          ...
          if (value.length) {
            jQuery(".shp__cart__wrap").css({"display": "block"});
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<div class="single-product-cart fix"><div class="cart-img"><a href="' + item.url + '"><img src="' + item.image + '" alt=""></a></div><div class="cart-title"><h5><a href="' + item.url + '">' + item.title + '</a></h5><span>' + Shopify.formatMoney(item.price, formatMoney) + ' x ' + item.quantity + '</span></div><div class="cart-delete"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')"><i class="ti-trash"></i></a></div></div>').appendTo(table);
            });
          }
          else {
            jQuery(".shp__cart__wrap").css({"display": "none"});
